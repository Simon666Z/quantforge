...
import { Settings2, DollarSign, ShieldAlert, ChevronDown, ChevronUp, Lock, Save } from 'lucide-react';
...
const ParamSlider: React.FC<ParamSliderProps> = ({ 
  label, value, onChange, min, max, step = 1, colorTheme = 'sakura', suffix = '' 
    ...
    const colorStyles: any = {
        ...
        amber:  { badge: 'bg-amber-50 text-amber-600',    rangeClass: 'range-accent-slate' },
    ...
    return (
        <div className="group">
          ...
          <div className="relative h-6 flex items-center">
            <input type="range" min={min} max={max} step={step} value={localValue} onChange={handleInput} onMouseUp={handleCommit} onTouchEnd={handleCommit} className={`range-slider ${theme.rangeClass}`} />
          </div>
        </div>
    );
};
...
export const ConfigPanel: React.FC<ConfigPanelProps> = ({
  strategy, setStrategy, params, setParams, fees, setFees, slippage, setSlippage, onSaveStrategy
}) => {
      ...
      <div className="space-y-6 pr-1">
        <div className="relative z-20">
           ...
           <div className="relative group focus-within:scale-[1.02] transition-transform duration-300 ease-out origin-left">
             ...
             <input type="number" value={params.initialCapital} onChange={(e) => handleParamChange('initialCapital', parseInt(e.target.value) || 0)} className="w-full pl-8 pr-4 py-2 rounded-xl border border-slate-200 focus:border-sakura-400 focus:ring-4 focus:ring-sakura-50 outline-none text-slate-600 bg-slate-50/50 font-mono transition-all duration-300 hover:border-sakura-200" />
           </div>
        </div>
        ...
        <div className="relative z-10"><Label>Strategy Model</Label><StrategySelector value={strategy} onChange={setStrategy} /></div>

        <div className="space-y-8 bg-white p-6 rounded-2xl border border-sakura-100 shadow-sm relative z-0">
          <Label>Parameters</Label>
          {(strategy === StrategyType.SMA_CROSSOVER || strategy === StrategyType.EMA_CROSSOVER) && <><ParamSlider label="Short Window" value={params.shortWindow} onChange={(v) => handleParamChange('shortWindow', v)} min={5} max={50} suffix="D" colorTheme="sakura" /><ParamSlider label="Long Window" value={params.longWindow} onChange={(v) => handleParamChange('longWindow', v)} min={20} max={200} suffix="D" colorTheme="sky" /></>}
          {strategy === StrategyType.RSI_REVERSAL && <><ParamSlider label="RSI Period" value={params.rsiPeriod} onChange={(v) => handleParamChange('rsiPeriod', v)} min={2} max={30} colorTheme="sakura" /><ParamSlider label="Oversold" value={params.rsiOversold} onChange={(v) => handleParamChange('rsiOversold', v)} min={10} max={40} colorTheme="emerald" /><ParamSlider label="Overbought" value={params.rsiOverbought} onChange={(v) => handleParamChange('rsiOverbought', v)} min={60} max={90} colorTheme="rose" /></>}
          {strategy === StrategyType.BOLLINGER_BANDS && <><ParamSlider label="Period" value={params.bbPeriod} onChange={(v) => handleParamChange('bbPeriod', v)} min={5} max={50} colorTheme="sakura" /><ParamSlider label="Std Dev" value={params.bbStdDev} onChange={(v) => handleParamChange('bbStdDev', v)} min={1} max={4} step={0.1} colorTheme="sakura" /></>}
          {strategy === StrategyType.MACD && <><ParamSlider label="Fast EMA" value={params.macdFast} onChange={(v) => handleParamChange('macdFast', v)} min={5} max={50} colorTheme="sakura" /><ParamSlider label="Slow EMA" value={params.macdSlow} onChange={(v) => handleParamChange('macdSlow', v)} min={20} max={100} colorTheme="sakura" /><ParamSlider label="Signal" value={params.macdSignal} onChange={(v) => handleParamChange('macdSignal', v)} min={5} max={20} colorTheme="sakura" /></>}
          {strategy === StrategyType.MOMENTUM && <ParamSlider label="Lookback Period" value={params.rocPeriod} onChange={(v) => handleParamChange('rocPeriod', v)} min={5} max={50} colorTheme="sakura" />}
          {strategy === StrategyType.TREND_RSI && <><ParamSlider label="Trend SMA" value={params.trendMa} onChange={(v) => handleParamChange('trendMa', v)} min={50} max={365} suffix="D" colorTheme="slate" /><ParamSlider label="RSI Period" value={params.rsiPeriod} onChange={(v) => handleParamChange('rsiPeriod', v)} min={2} max={30} colorTheme="sakura" /></>}
          {strategy === StrategyType.TURTLE && <><ParamSlider label="Entry Breakout" value={params.turtleEntry} onChange={(v) => handleParamChange('turtleEntry', v)} min={10} max={100} suffix="D" colorTheme="emerald" /><ParamSlider label="Exit Breakdown" value={params.turtleExit} onChange={(v) => handleParamChange('turtleExit', v)} min={5} max={50} suffix="D" colorTheme="rose" /></>}
          {strategy === StrategyType.VOLATILITY_FILTER && <><ParamSlider label="ADX Period" value={params.adxPeriod} onChange={(v) => handleParamChange('adxPeriod', v)} min={5} max={50} colorTheme="slate" /><ParamSlider label="Min Strength" value={params.adxThreshold} onChange={(v) => handleParamChange('adxThreshold', v)} min={10} max={50} colorTheme="indigo" /></>}
          {strategy === StrategyType.KELTNER && <><ParamSlider label="EMA Period" value={params.keltnerPeriod} onChange={(v) => handleParamChange('keltnerPeriod', v)} min={10} max={50} colorTheme="sky" /><ParamSlider label="ATR Multiplier" value={params.keltnerMult} onChange={(v) => handleParamChange('keltnerMult', v)} min={1.0} max={4.0} step={0.1} colorTheme="sakura" /></>}
        </div>
      </div>
    </Card>
  );
