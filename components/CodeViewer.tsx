...
export const CodeViewer: React.FC<CodeViewerProps> = ({ isOpen, onClose, codes }) => {
  ...
  // Handle open animation state
  useEffect(() => {
    if (isOpen) setIsMounting(true);
    else setTimeout(() => setIsMounting(false), 300);
  }, [isOpen]);
  ...
  return (
    <div className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
      ...
      {/* Modal Window with Bouncy Scale */}
      <div 
        className={`bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[85vh] relative border border-white/50 ring-1 ring-black/5 overflow-hidden transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1) ${
            isOpen 
      ...
      >
        ...
        <div className="flex justify-between items-center p-4 border-b border-slate-100 bg-slate-50/50">
          ...
          <button 
            onClick={onClose} 
            className="p-2 hover:bg-rose-50 hover:text-rose-500 rounded-full transition-colors text-slate-400 btn-bouncy"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex border-b border-slate-100 bg-white">
          <TabButton 
            active={activeTab === 'pseudo'} 
            onClick={() => setActiveTab('pseudo')} 
            icon={BookOpen} 
            label="Logic (Whitebox)" 
          />
          <TabButton 
            active={activeTab === 'vbt'} 
            onClick={() => setActiveTab('vbt')} 
            icon={FileCode} 
            label="VectorBT (Python)" 
          />
          <TabButton 
            active={activeTab === 'bt'} 
            onClick={() => setActiveTab('bt')} 
            icon={Terminal} 
            label="Backtrader (Python)" 
          />
        </div>

        {/* Code Content - Fixed Overflow and Animation */}
        <div className="flex-1 relative bg-[#282c34] group overflow-hidden">
           {/* Code Text with fade transition */}
           <div key={activeTab} className="h-full overflow-auto custom-scrollbar animate-in fade-in slide-in-from-bottom-2 duration-300">
           ...
           <button 
             onClick={handleCopy}
             className="absolute top-4 right-4 flex items-center gap-2 px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white text-xs font-bold rounded-lg backdrop-blur-md transition-all border border-white/10 shadow-lg hover:scale-105 active:scale-95"
           >
